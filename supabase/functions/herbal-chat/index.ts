...
async function findRelevantKnowledge(supabase: any, question: string, enableMahidol = true, enableTu = true): Promise<KnowledgeDoc[]> {
  ...
  if (!enableTu) {
    queryBuilder = queryBuilder.neq("category", "อันตรกิริยาระหว่างยาและสมุนไพร (DDI - ม.ธรรมศาสตร์)");
  }
  ...
  if (error) {
    ...
    if (!enableTu) {
      fallbackBuilder = fallbackBuilder.neq("category", "อันตรกิริยาระหว่างยาและสมุนไพร (DDI - ม.ธรรมศาสตร์)");
    }
...
/** กรองเอกสาร DDI ของ ม.มหิดล และ ม.ธรรมศาสตร์ ให้ตรงกับสมุนไพรและยาที่ผู้ใช้ถามจริง ห้ามปนสมุนไพรอื่น */
function filterStrictDdiKnowledge(
  ...
  return docs.filter((k) => {
    const isMahidolDdi = k.category === "อันตรกิริยาระหว่างยาและสมุนไพร (DDI)";
    const isTuDdi = k.category === "อันตรกิริยาระหว่างยาและสมุนไพร (DDI - ม.ธรรมศาสตร์)";
    if (!isMahidolDdi && !isTuDdi) return true;

    const m = (k.title || "").match(/อันตรกิริยาระหว่าง\s+(.+?)\s+กับ\s+(.+?)(?:\s+\((?:ม\.มหิดล|ม\.ธรรมศาสตร์)\))?$/);
...
กติกา:
- in_scope = true สำหรับทุกคำถามที่เกี่ยวกับ: การแพทย์แผนไทย สมุนไพร ตำรับยาไทย, การแพทย์แผนปัจจุบัน ยาทุกชนิด, อาการเจ็บป่วยและการดูแลตนเอง (เช่น "ปวดท้องกินไรดี", "นอนไม่หลับ", "ปวดหัว"), ขนาดยา ข้อห้าม ผลข้างเคียง, drug interaction, แหล่งข้อมูล/แหล่งอ้างอิง/ฐานข้อมูล/ที่มาของข้อมูลและการตรวจสอบข้อมูลของระบบหมอยาพิษณุโลก, หรือคำถามต่อเนื่องจากบทสนทนาเดิม
- in_scope = false เมื่อคำถามไม่ได้เกี่ยวกับทางด้านการแพทย์แผนไทย การแพทย์แผนปัจจุบัน หรือการดูแลสุขภาพ เช่น การเขียนโปรแกรม/โค้ดดิ้ง, การเมือง/เลือกตั้ง, กีฬา/ฟุตบอล, พยากรณ์อากาศ, ดูดวง/โหราศาสตร์/หวย, แปลภาษาทั่วไป, บันเทิง/เพลง/ภาพยนตร์, ช่าง/ซ่อมแซม, การเงิน/หุ้น, เรื่องส่วนตัวทั่วไปของ AI
...
function sanitizeMahidolReferences(content: string): string {
...
function sanitizeTuReferences(content: string): string {
  if (!content) return content;
  const lines = content.split("\n");
  const filteredLines = lines.filter((line) => {
    if (
...
function buildSystemPrompt(enableMahidol = true, enableTu = true): string {
  const verifyItems = [
    enableMahidol ? "ม.มหิดล" : "",
    enableTu ? "ม.ธรรมศาสตร์" : "",
    "บัญชียาหลักฯ",
    "10 กลุ่มอาการ สธ.",
    "PubMed",
    "ThaiJO",
    "ทีมเภสัชกร สสจ.พิษณุโลก",
  ].filter(Boolean);
  const scopeVerification = verifyItems.join(", ");
  ...
  const tuApaRule = enableTu
    ? `  - ฐานข้อมูลข้อควรระวังอันตรกิริยาระหว่างสมุนไพรกับยาแผนปัจจุบัน ม.ธรรมศาสตร์: อิฐรัตน์, อ. (2566). *ข้อควรระวังอันตรกิริยาระหว่างสมุนไพรกับยาแผนปัจจุบัน (Herb-Drug Interaction)*. สถานการแพทย์แผนไทยประยุกต์ คณะแพทยศาสตร์ มหาวิทยาลัยธรรมศาสตร์.`
    : `  - ⚠️ **ข้อห้ามเด็ดขาดเรื่อง ม.ธรรมศาสตร์:** ปัจจุบันระบบปิดการใช้ฐานข้อมูลอันตรกิริยาระหว่างสมุนไพรกับยาแผนปัจจุบันของ ม.ธรรมศาสตร์ ห้ามเอ่ยถึง อ้างอิง หรือระบุชื่อ "มหาวิทยาลัยธรรมศาสตร์", "ม.ธรรมศาสตร์", "ศ. ดร.ภญ.อรุณพร อิฐรัตน์", "อรุณพร อิฐรัตน์" หรือ "สถานการแพทย์แผนไทยประยุกต์" ในคำตอบและในหัวข้อเอกสารอ้างอิง (APA 7th Edition) โดยเด็ดขาด!`;

  return `คุณคือผู้เชี่ยวชาญด้านยาสมุนไพรไทยและ Drug-Herb Interaction ของ "กลุ่มงานการแพทย์แผนไทยและสมุนไพร สำนักงานสาธารณสุขจังหวัดพิษณุโลก"`
...
- **แหล่งข้อมูล แหล่งอ้างอิง และฐานข้อมูลที่ระบบใช้ (System Knowledge Sources & Verification):** ที่มาของข้อมูลอันตรกิริยา แหล่งตรวจสอบย้อนกลับ (${scopeVerification}) **ต้องตอบอย่างละเอียดและชัดเจน ห้ามปฏิเสธเด็ดขาด**
...
## รูปแบบคำตอบ
- **หลักการยึดถือข้อมูลจากฐานความรู้และการแยกแยะระดับข้อมูล (สำคัญสูงสุด):**
  - **ใช้ข้อมูลจากเอกสารในฐานความรู้เป็นหลัก:** ให้ตอบคำถามโดยอิงข้อมูลจากเอกสารในฐานความรู้ (CONTEXT) เป็นหลักอย่างเคร่งครัด
  ...
  - **ต้องแยกให้ชัดเจนระหว่างประเภทของข้อมูล 3 ระดับ:**
    1) **ข้อมูลที่ระบุโดยตรงในเอกสาร (Direct Evidence):** เช่น สรรพคุณ ข้อบ่งใช้ ขนาดยา วิธีใช้ ข้อห้ามใช้ หรือรายงานผลข้างเคียงที่มีระบุไว้ชัดเจนในเอกสารกำกับยา ประกาศกระทรวง หรือรายงานวิจัย
    2) **ข้อมูลที่สังเคราะห์จากหลายแหล่ง (Synthesized Information):** ข้อมูลที่มีการประมวล เปรียบเทียบ หรือสรุปรวมจากแหล่งข้อมูลหลายแห่งเข้าด้วยกัน ให้ระบุชัดเจนว่าเป็นการประมวล/สังเคราะห์จากหลายแหล่ง
    3) **ข้อมูลที่เป็นเพียงกลไกหรือข้อสันนิษฐานทางทฤษฎี (Theoretical / Mechanistic Data):** ข้อมูลที่เป็นเพียงกลไกทางเภสัชวิทยาในระดับเซลล์/หลอดทดลอง (เช่น ฤทธิ์ยับยั้งเอนไซม์ CYP450) หรือการศึกษาในสัตว์ทดลอง ที่ยังไม่มีรายงานยืนยันผลทางคลินิกในมนุษย์ **ต้องระบุให้ชัดเจนเสมอว่าเป็น "กลไกหรือข้อสันนิษฐานทางทฤษฎี" หรือ "ข้อมูลจากการศึกษาในระดับหลอดทดลอง/สัตว์ทดลอง ซึ่งยังไม่มีรายงานยืนยันในมนุษย์"** เพื่อไม่ให้ผู้ใช้เข้าใจผิด
- **ตอบตรงประเด็นและกระชับเป็นอันดับแรก (Concise & Directly Answering):**
  - วิเคราะห์เจตนาของคำถาม และตอบประเด็นที่ถามเป็นหลักอย่างชัดเจน ไม่เกริ่นนำยืดยาว และไม่พูดซ้ำซ้อน
  - **หากผู้ใช้ถามเจาะจงเฉพาะเรื่องใดเรื่องหนึ่ง** (เช่น ข้อบ่งใช้, ขนาดยา, หญิงตั้งครรภ์ใช้ได้ไหม, ผลข้างเคียง):
    -> ตอบเฉพาะประเด็นที่ถามให้ตรงเป้าหมายทันที
    -> **ห้าม** ยกเทมเพลตข้อมูลอื่นที่ไม่เกี่ยวข้องมาตอบทั้งหมด (เช่น ถามแค่ข้อบ่งใช้ ไม่ต้องแถมขนาดยาเต็มสูตร หรือตารางอันตรกิริยายาวๆ เว้นแต่มีข้อควรระวังสำคัญต่อชีวิตที่ต้องเตือนสั้นๆ 1 บรรทัด)
  - **หากผู้ใช้ถามภาพรวมของสมุนไพร/ตำรับยา** (เช่น "ขอข้อมูลฟ้าทะลายโจร", "ยาประสะไพลคืออะไร มีสรรพคุณและวิธีใช้อย่างไร"):
    -> สรุปข้อมูลครบถ้วน: สรรพคุณ, ขนาดและวิธีใช้, ข้อห้าม/ข้อควรระวังสำคัญ อย่างเป็นสัดส่วน กระชับ อ่านเข้าใจง่าย
- **การแสดงตารางอันตรกิริยากับยาแผนปัจจุบัน (Drug-Herb Interaction) และกฎเรื่องระดับความรุนแรง:**
  ...
  - หากผู้ใช้ถามเรื่องทั่วไป **ห้าม** แสดงตารางอันตรกิริยาขนาดใหญ่เข้ามาโดยไม่จำเป็น หากมีข้อควรระวังเรื่องยาอื่น ให้ระบุสั้นๆ 1 บรรทัดในหัวข้อข้อควรระวังก็เพียงพอ
...
- **แสดงเอกสารอ้างอิงตามมาตรฐาน APA 7th Edition:**
  ...
  - ข้อกำหนดเรื่องฐานข้อมูลภายใน: ให้ยังคงใช้ข้อมูลสรรพคุณ ขนาด วิธีใช้ และข้อควรระวังจากฐานข้อมูลยาภายในตามปกติ แต่ **ไม่ต้องแสดงรายการอ้างอิง "สำนักงานสาธารณสุขจังหวัดพิษณุโลก" ในหัวข้อเอกสารอ้างอิง** (ให้ซ่อนรายการอ้างอิงของ สสจ.พิษณุโลก ไว้)
${mahidolApaRule}
${tuApaRule}
...
- ปิดท้ายด้วยคำเตือนสั้นๆ: "💡 ข้อแนะนำ: ควรปรึกษาแพทย์หรือเภสัชกรก่อนใช้ โดยเฉพาะหญิงตั้งครรภ์ ให้นมบุตร หรือผู้มีโรคประจำตัว"
...
const SYSTEM_PROMPT = buildSystemPrompt(true);
...
serve(async (req) => {
  ...
  try {
    ...
    const enableTu = settings?.enable_tu_ddi !== false;
    ...
    // รันการค้นหาแบบขนาน (DB + knowledge) เฉพาะเมื่อเปิดใช้งานฐานข้อมูลภายในหรือฐานข้อมูลมหิดลหรือธรรมศาสตร์
    const knowledgeQuery = [question, ...intent.symptoms, ...intent.herbs, ...intent.drugs].join(" ");
    const [{ herbs, formulas, nameMatchedHerbNames, nameMatchedFormulaNames, listMode }, rawKnowledge] = (enableInternal || enableMahidol || enableTu)
      ? await Promise.all([
          enableInternal
          ...
          findRelevantKnowledge(supabase, knowledgeQuery, enableMahidol, enableTu),
        ])
      : [
    ...
    // กรองเอกสาร DDI ให้ตรงกับสมุนไพรและยาที่ผู้ใช้ถามจริง ห้ามปนสมุนไพรอื่น
    let filteredKnowledge = rawKnowledge;
    if (!enableMahidol) {
    ...
    const knowledge = (enableMahidol || enableTu)
      ? filterStrictDdiKnowledge(question, filteredKnowledge, intent.herbs, intent.drugs)
      : filteredKnowledge;
    ...
    const knowledgeSources: KnowledgeSource[] = (enableInternal || enableMahidol || enableTu)
      ? knowledge
          .filter((k) => {
            if (!enableMahidol && (
              k.category === "อันตรกิริยาระหว่างยาและสมุนไพร (DDI)" ||
    ...
    const contextBlock = buildContext(
      ...
      (enableInternal || enableMahidol || enableTu) ? knowledge : [],
    ...
    const sourcesJson = JSON.stringify({
      ...
      knowledge: (enableInternal || enableMahidol || enableTu) ? knowledgeSources : [],
    ...
    const listInstruction = listMode && (formulas.length > 0 || herbs.length > 0)
      ? `\n\nคำสั่งพิเศษ: ผู้ใช้ต้องการ "รายชื่อ" ตำรับยาหรือสมุนไพร — ให้ลิสต์รายชื่อยาที่มีใน CONTEXT ให้ครบถ้วน ชัดเจน เป็นข้อ ๆ (bullet points) พร้อมระบุข้อบ่งใช้สั้น ๆ ของแต่ละตำรับ ห้ามสรุปคลุมเครือ และห้ามตอบว่าไม่ได้ระบุชื่อ`
      : "";
    ...
    if (!enableTu) {
      settingsInstruction += "\n\n⚠️ ข้อห้ามเด็ดขาด: ขณะนี้ระบบปิดการใช้ฐานข้อมูลอันตรกิริยาระหว่างสมุนไพรกับยาแผนปัจจุบันของ ม.ธรรมศาสตร์ (Herb-Drug Interaction) ห้ามนำข้อมูล DDI ธรรมศาสตร์มาใช้ และห้ามเอ่ยถึง อ้างอิง หรือระบุชื่อ 'มหาวิทยาลัยธรรมศาสตร์', 'ม.ธรรมศาสตร์', 'ศ. ดร.ภญ.อรุณพร อิฐรัตน์', 'อรุณพร อิฐรัตน์' หรือ 'สถานการแพทย์แผนไทยประยุกต์' ในเนื้อหาคำตอบและในหัวข้อ '📚 เอกสารอ้างอิง (APA 7th Edition)' โดยเด็ดขาด!";
    }
...
จำไว้: อ้างอิงเฉพาะจาก CONTEXT ข้างต้นเท่านั้น ห้ามแต่งแหล่งอ้างอิงใหม่ และเวลาใส่ [SOURCES] ให้คัดลอก JSON ในแท็ก <แหล่งอ้างอิงที่ใช้จริง> ทั้งหมดโดยไม่แก้ไข${listInstruction}${scopeInstruction}${settingsInstruction}`,
    ...
    try {
      const draftResult = await aiComplete(providers, {
        messages: [
          { role: 'system', content: buildSystemPrompt(enableMahidol, enableTu) },
          contextMessage,
...
7. ${!enableMahidol ? "ห้ามมีชื่อ ม.มหิดล หรือศูนย์ข้อมูลสมุนไพรในคำตอบหรือในหัวข้อเอกสารอ้างอิงเด็ดขาดเนื่องจากระบบปิดใช้งานอยู่" : "ห้ามใส่แหล่งอ้างอิงที่ไม่มีใน CONTEXT"}${!enableTu ? " และห้ามมีชื่อ ม.ธรรมศาสตร์ หรือ อ.อรุณพร ในคำตอบหรือในหัวข้อเอกสารอ้างอิงเด็ดขาดเนื่องจากระบบปิดใช้งานอยู่" : ""}
...
- ถ้าไม่ถูกต้อง ให้ส่ง "คำตอบฉบับแก้ไข" ฉบับเต็ม (ภาษาไทย Markdown พร้อมบล็อก [METADATA] และ [SOURCES]) โดยไม่ต้องอธิบายเหตุผลใด ๆ`,
    ...
    // กรองการอ้างอิง ม.มหิดล หากปิดการใช้งาน
    if (!enableMahidol) {
      finalAnswer = sanitizeMahidolReferences(finalAnswer);
    }
    // กรองการอ้างอิง ม.ธรรมศาสตร์ หากปิดการใช้งาน
    if (!enableTu) {
      finalAnswer = sanitizeTuReferences(finalAnswer);
    }
