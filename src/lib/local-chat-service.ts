...
export function sanitizeUnrelatedApaReferences(
  ...
  const lowerAllowed = entities.map((e) => e.toLowerCase().trim()).filter((e) => e.length >= 2);
  const normAllowed = entities.map((e) => normalizeThaiName(e)).filter((e) => e.length >= 2);
  ...
  for (const line of lines) {
    ...
    if (inCitationBlock && line.trim().length > 0) {
      ...
      // ตรวจสอบกรณีอ้างอิง CPG กรมการแพทย์ (2568): หากระบุตัวยาแต่ตัวยานั้นไม่มีใน CPG และไม่มี CPG ใน sources แสดงว่าหลอนขึ้นมา
      const isCpgCitation = /กรมการแพทย์.*(?:2568|คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ)/i.test(line);
      const isCpgAllowed = lowerAllowed.some((ent) =>
        ent.includes("คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ") ||
        ent.includes("กรมการแพทย์") ||
        ent.includes("cpg") ||
        ent.includes("25680421113642am")
      );
      if (isCpgCitation && lowerAllowed.length > 0 && !isCpgAllowed) {
        continue;
      }
      ...
      if (isAllowed || isGeneralOfficialDoc) {
        // บังคับใส่เลขลำดับ 1., 2., 3. ด้านหน้าทุกข้อ โดยตัดเลขเดิมหรือ bullet points (-, *, •) ออกก่อน
        const cleanLine = line.trim().replace(/^(?:[-*•\s]|\[?\d+\]?[.)]?)*\s*/, "");
        filteredLines.push(`${citationIndex}. ${cleanLine}`);
        citationIndex++;
      } else {
        continue;
      }
  ...
  // หากรายการอ้างอิงว่างเปล่าหลังตัด CPG ที่หลอนออก ให้ใส่อ้างอิงบัญชียาหลักแห่งชาติเป็นแกนหลัก
  const remainingCitations = filteredLines.filter(
    (l) => l.trim().length > 0 && !/(?:###\s*)?📚\s*เอกสารอ้างอิง/i.test(l)
  );
  if (remainingCitations.length === 0 && inCitationBlock) {
    filteredLines.push("1. คณะกรรมการพัฒนาระบบยาแห่งชาติ. (2568). *ประกาศคณะกรรมการพัฒนาระบบยาแห่งชาติ เรื่อง บัญชียาหลักแห่งชาติด้านสมุนไพร (ฉบับที่ 2) พ.ศ. 2568*. ราชกิจจานุเบกษา.");
    filteredLines.push("2. กรมการแพทย์แผนไทยและการแพทย์ทางเลือก. (2567). *คู่มือการใช้ยาสมุนไพรในการดูแลสุขภาพเบื้องต้น 10 กลุ่มอาการ*. กระทรวงสาธารณสุข.");
  }
...
/** ตัดหัวข้อและรายการเอกสารอ้างอิง APA 7th Edition ทั้งหมดออกจากเนื้อหาข้อความ (เมื่อ admin ปิดการแสดงผล) รวมถึง in-text numbered citations [1], [2] */
export function stripAllApaReferences(content: string): string {
  ...
  let cleaned = content
    ...
    .trim();
  // ลบ in-text citation เช่น [1], [2], [1, 2], [1][2] ออกจากเนื้อหาด้วยเมื่อปิดการแสดงผล APA
  cleaned = cleaned.replace(/(?:\s*(?:,\s*)?\[\d+(?:,\s*\d+)*\](?!\())+/g, "");
  cleaned = cleaned.replace(/\n*\s*---\s*$/, "").trim();
  return cleaned;
}
...
export function buildSystemPrompt(settings?: KnowledgeSettings): string {
  ...
  const herbBooksApaRule = enableHerbBooks
    ? `    - กรณีอ้างอิงหนังสือและแนวทางเวชปฏิบัติ (ให้อ้างอิงเฉพาะเล่มที่สมุนไพรนั้นมีบันทึกอยู่จริงและนำมาใช้ตอบจริง ห้ามอ้างอิงเล่มที่ไม่มีข้อมูลสมุนไพรนั้นเด็ดขาด โดยใส่ตัวเลขลำดับ 1., 2., 3. ด้านหน้าเสมอ):
      - หากสมุนไพรนั้นมีข้อมูลใน CPG 2568:
        1. กรมการแพทย์. (2568). *คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ*. กระทรวงสาธารณสุข.
      - หากสมุนไพรนั้นเป็นตำรับยา/สมุนไพรในบัญชียาหลักแห่งชาติแต่ไม่มีใน CPG 2568 (เช่น ยาห้าราก, ยาประสะไพล, มะขามแขก ฯลฯ):
        ...
        (ห้ามใส่ "กรมการแพทย์. (2568)" เด็ดขาด)
      - เอกสารเสริมอื่นๆ (เมื่อนำมาใช้ตอบจริง):
        2. กรมการแพทย์แผนไทยและการแพทย์ทางเลือก. (2567). *แนวทางการใช้ยาสมุนไพรในบัญชียาหลักแห่งชาติทดแทนยาแผนปัจจุบันใน 10 กลุ่มโรคสำคัญ*. กระทรวงสาธารณสุข.
        3. กรมการแพทย์แผนไทยและการแพทย์ทางเลือก. (2567). *แนวทางการรักษาอาการเจ็บป่วยด้วยยาสมุนไพรในระบบบริการปฐมภูมิ*. กระทรวงสาธารณสุข.
        4. คณะกรรมการพัฒนาระบบยาแห่งชาติ. (2568). *ประกาศคณะกรรมการพัฒนาระบบยาแห่งชาติ เรื่อง บัญชียาหลักแห่งชาติด้านสมุนไพร (ฉบับที่ 2) พ.ศ. 2568*. ราชกิจจานุเบกษา.
        5. กรมการแพทย์แผนไทยและการแพทย์ทางเลือก. (2567). *คู่มือการใช้ยาสมุนไพรในการดูแลสุขภาพเบื้องต้น 10 กลุ่มอาการ*. กระทรวงสาธารณสุข.
        6. กลุ่มงานการแพทย์แผนไทยและการแพทย์ทางเลือก สำนักงานสาธารณสุขจังหวัดบุรีรัมย์. (2567). *กลุ่มอาการของโรคที่พบบ่อยกับการใช้ยาสมุนไพรในบัญชียาหลักแห่งชาติ* [แผ่นภาพความรู้ A5]. กระทรวงสาธารณสุข.
        7. กลุ่มงานการแพทย์แผนไทยและการแพทย์ทางเลือก สำนักงานสาธารณสุขจังหวัดบุรีรัมย์. (2567). *ยาสมุนไพรในกลุ่มอาการที่พบบ่อย ยาทดแทน 19 รายการ* [แผ่นภาพความรู้ A5]. กระทรวงสาธารณสุข.`
  ...
  const apaSectionBlock = showApa
    ? `5. **การอ้างอิงในเนื้อหาคำตอบด้วยระบบตัวเลข และการแสดงเอกสารอ้างอิงตามแบบ APA 7th Edition (สำคัญที่สุด):**
    - **การใส่หมายเลขกำกับอ้างอิงในเนื้อหา (In-Text Numbered Citations [1], [2] — บังคับใช้):**
      - ในเนื้อหาของคำตอบ (เช่น สรรพคุณ, ข้อบ่งใช้, ขนาดและวิธีใช้, การใช้ทดแทนยาแผนปัจจุบัน, ข้อห้าม/ข้อควรระวัง, หรืออันตรกิริยาระหว่างยา) **ต้องใส่หมายเลขในวงเล็บก้ามปู เช่น [1], [2] กำกับท้ายประโยคหรือข้อความทางคลินิกสำคัญเสมอ** เพื่อให้แพทย์ พยาบาล และเภสัชกรสามารถสืบค้นย้อนกลับไปยังเอกสารต้นทางได้อย่างแม่นยำ
      - หมายเลขกำกับในเนื้อหา [1], [2], [3] ต้องสอดคล้องตรงกับลำดับรายการในหัวข้อ "### 📚 เอกสารอ้างอิง (APA 7th Edition)" แบบ 1 ต่อ 1 เสมอ:
        * [1] = มาจากแหล่งข้อมูลหลัก (Primary Core Authority เช่น CPG กรมการแพทย์ 2568, คู่มือ 32 ยาทดแทน, หรือประกาศบัญชียาหลัก)
        * [2], [3] = มาจากแหล่งข้อมูลเสริม (Supplementary Sources เช่น แผนภูมิปฐมภูมิ, CD 10 กลุ่มโรค, แผ่นภาพ สสจ.บุรีรัมย์, หรือวารสารวิจัย)
    - **ก่อนจบคำตอบ ให้เขียนหัวข้อ "### 📚 เอกสารอ้างอิง (APA 7th Edition)"** แล้วระบุรายการอ้างอิงตามรูปแบบมาตรฐาน APA 7 เฉพาะรายการเอกสารต้นทางหรือวิจัยที่เกี่ยวข้องโดยตรงกับคำถามและนำมาใช้ตอบจริง:
      - **กฎสำคัญที่สุดเรื่องการใส่ตัวเลขลำดับด้านหน้าทุกข้อ (Numbered Citation Items 1., 2., 3. — บังคับใช้เด็ดขาด):** ในหัวข้อ "### 📚 เอกสารอ้างอิง (APA 7th Edition)" **ทุกรายการอ้างอิงต้องใส่ตัวเลขลำดับ เช่น 1., 2., 3. ด้านหน้าเสมอ** (ห้ามใช้เครื่องหมายขีด - หรือดอกจัน * หรือเขียนโดยไม่มีตัวเลขด้านหน้าเด็ดขาด) โดยเรียงลำดับตัวเลข 1., 2., 3. ให้ตรงกับหมายเลข [1], [2], [3] ในเนื้อหาคำตอบ ระบุแหล่งข้อมูลหลัก (Primary Source [1]) เป็นอันดับแรก 1. เสมอ และตามด้วยแหล่งข้อมูลเสริม ([2], [3]) ทั้งหมดที่นำมาใช้ตอบจริง
...
${herbBooksApaRule}
   ...
   - ⚠️ **คำสั่งสำคัญ:** ขณะนี้ระบบปิดการแสดงผลหัวข้อเอกสารอ้างอิง APA 7th Edition **ห้ามใส่หัวข้อ "### 📚 เอกสารอ้างอิง (APA 7th Edition)" หรือรายการอ้างอิง APA ใดๆ ท้ายคำตอบเด็ดขาด และห้ามใส่หมายเลขกำกับอ้างอิง [1], [2] ในเนื้อหาคำตอบ** ให้ตอบเฉพาะเนื้อหาคำแนะนำและแท็ก [METADATA] เท่านั้น`;
...
3. **การแสดงตารางอันตรกิริยากับยาแผนปัจจุบัน และกฎเรื่องระดับความรุนแรง (Drug-Herb Interaction & Severity Rules):**
   ...
   - **กฎเหล็กเด็ดขาดเรื่องระดับความรุนแรง (ห้ามกำหนดเองเด็ดขาด):**
    ...
    - **กฎสำคัญสูงสุดเรื่องแหล่งข้อมูลอันตรกิริยาระหว่างสมุนไพรกับยา (Herb-Drug Interactions Exclusive Source Rule):**
      ...
      - ให้ใส่หมายเลข [1] กำกับท้ายประโยคหรือตารางอันตรกิริยา และในหัวข้อ "### 📚 เอกสารอ้างอิง (APA 7th Edition)" ให้ระบุรายการที่ 1 คือ:
        1. กรมการแพทย์. (2568). *คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ*. กระทรวงสาธารณสุข.
...
export async function processLocalChat(
  ...
  // 0.1 ตรวจสอบคำถามที่ตรงกับข้อมูลมาตรฐานที่ผ่านการตรวจทานและรับรองแล้ว (Verified Benchmark Knowledge Fast-Path)
  const verifiedMatch = findVerifiedAnswer(question);
  if (verifiedMatch.found && verifiedMatch.verifiedAnswer) {
    let cleanAnswer = verifiedMatch.verifiedAnswer;
    ...
    const primaryHerb = matched97[0]?.name || (verifiedMatch.item?.tags?.[0]) || "";
    const metaCategory = verifiedMatch.item?.category === "cannabis" ? "cannabis" : "herbal_info";

    const verifiedSources: any = {
      internal: matched97.map((h) => ({ type: "formula", id: h.id, name: h.name })),
      pubmed: [],
      thaijo: [],
      knowledge: [],
    };

    if (cleanAnswer.includes("ประกาศคณะกรรมการพัฒนาระบบยาแห่งชาติ")) {
      const docHerb = matched97[0]?.name || primaryHerb;
      verifiedSources.knowledge.push({
        id: "nlem-verified-doc",
        title: `ประกาศคณะกรรมการพัฒนาระบบยาแห่งชาติ เรื่อง บัญชียาหลักแห่งชาติด้านสมุนไพร: ${docHerb}`,
        category: "บัญชียาหลักแห่งชาติด้านสมุนไพร",
        source: "คณะกรรมการพัฒนาระบบยาแห่งชาติ",
        source_url: docHerb ? `/herbs?name=${encodeURIComponent(docHerb)}` : "/herbs",
      });
    }

    if (cleanAnswer.includes("10 กลุ่มอาการ")) {
      verifiedSources.knowledge.push({
        id: "manual-10-symptoms",
        title: "คู่มือการใช้ยาสมุนไพรในการดูแลสุขภาพเบื้องต้น 10 กลุ่มอาการ (พ.ศ. 2567)",
        category: "คู่มือ 10 กลุ่มอาการ",
        source: "กรมการแพทย์แผนไทยและการแพทย์ทางเลือก",
        source_url: "/knowledge",
      });
    }

    if (cleanAnswer.includes("คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ")) {
      verifiedSources.knowledge.push({
        id: "cpg-2568-verified",
        title: "คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ กรมการแพทย์ (พ.ศ. 2568)",
        category: "หนังสือข้อมูลความรู้ด้านยาและเวชปฏิบัติ",
        source: "กรมการแพทย์",
        source_url: "/knowledge?tab=books&id=tier1-cpg-2568-mono-1",
      });
    }

    const metadataBlock = `\n\n[METADATA]\ncategory: ${metaCategory}\nseverity: none\nherbs: ${primaryHerb}\ndrugs:\n[/METADATA]`;
    const finalResponse = `${cleanAnswer}${metadataBlock}\n\n[SOURCES]${JSON.stringify(verifiedSources)}[/SOURCES]`;
  ...
  if (enableInternal) {
    ...
    if (allKnowledge.length > 0) {
      if (!isTierMatched && matchedHerbs.length === 0 && matchedFormulas.length === 0 && matched97Herbs.length === 0) {
  ...
  const fallbackVerifiedMatch = findVerifiedAnswer(question);
  if (fallbackVerifiedMatch.found && fallbackVerifiedMatch.verifiedAnswer) {
    contextText =
      `\n[ข้อมูลที่ผ่านการตรวจทานและรับรองความถูกต้องแล้ว (Verified Clinical Knowledge)]:\n` +
      `• เรื่อง/คำถาม: ${fallbackVerifiedMatch.docTitle || question}\n` +
      `• เนื้อหาที่ได้รับการรับรอง: ${fallbackVerifiedMatch.verifiedAnswer}\n` +
      `• แหล่งข้อมูลอ้างอิง: ${fallbackVerifiedMatch.source || "คลังข้อมูลสมุนไพรและประกาศบัญชียาหลักแห่งชาติด้านสมุนไพร"}\n` +
  ...
  const isSourceInquiry = /(?:แหล่ง(?:ข้อมูล|อ้างอิง|สืบค้น)|ที่มา(?:ของข้อมูล)?|ฐานข้อมูล|ตรวจสอบ(?:จาก|ได้จาก)?(?:แหล่ง|ที่)?|อ้างอิง(?:มา)?จาก(?:ไหน|ใด|แหล่ง)?|เอาข้อมูลมาจาก(?:ไหน|ใด)|น่าเชื่อถือ(?:ไหม|แค่ไหน|อย่างไร)|ระบบใช้(?:ข้อมูล|แหล่ง)|ใครเป็นผู้(?:พัฒนา|ให้ข้อมูล)|ตรวจทาน|รับรอง)/i.test(question);
  if (isSourceInquiry) {
    ...
    if (enableHerbBooks) {
      sourcesList.push(
        ...
        `   - การตรวจสอบย้อนกลับ: สามารถตรวจสอบข้อมูลต้นทางฉบับเต็มได้ที่ เมนู "เอกสารวิชาการ" ด้านบนของหน้าเว็บ (Google Drive) หรือดูในปุ่ม "แหล่งอ้างอิง" ของระบบ พร้อมรูปแบบการอ้างอิงตามมาตรฐาน APA 7th Edition`
      );
    }
    ...
    if (enableExternal) {
      sourcesList.push(
        ...
        `   - ระดับชาติ: ศูนย์ดัชนีการอ้างอิงวารสารไทย (ThaiJO / TCI) และฐานข้อมูลวิทยานิพนธ์และวิจัยไทย TDC (ThaiLIS)\n` +
        `   - การตรวจสอบย้อนกลับ: สามารถตรวจสอบข้อมูลต้นทางฉบับเต็มได้ที่ เมนู "เอกสารวิชาการ" ด้านบน หรือคลิกลิงก์เปิดเอกสารในปุ่ม "แหล่งอ้างอิง"`
      );
    }
    ...
    if (enableInternal) {
      sourcesList.push(
        ...
        `   - การตรวจสอบย้อนกลับ: ตรวจสอบข้อมูลต้นทางได้ที่ เมนู "เอกสารวิชาการ" ด้านบน หรือเมนู "คลังยาสมุนไพร" (/herbs)`
      );
    }
  ...
  if (isTierMatched) {
    ...
    dynamicInstructions += `\n\n⚠️ **กฎลำดับความสำคัญของแหล่งข้อมูลและการตอบประเด็นเสริม (Tiered Authority & Supplementary Rules):**\n` +
      `1. **ประเด็นคำถามหลัก (Primary Core):** คำถามนี้พบข้อมูลตอบคำถามจากแหล่งข้อมูลหลักลำดับที่ ${matchedTier}: "${sourceFileName}" ให้ใช้ข้อมูลจากแหล่งนี้เป็น **แกนหลักในการตอบประเด็นคำถามหลัก** อย่างเคร่งครัด โดยใส่หมายเลข [1] กำกับท้ายข้อความสำคัญที่มาจากแหล่งหลักนี้เสมอ\n` +
      `2. **การเสริมข้อมูลจากแหล่งอื่น (Supplementary):** หากมีประเด็นอื่นๆ ในคำตอบที่เสริมกับคำตอบ สามารถนำข้อมูลจากแหล่งเสริม (${suppSourcesText || "ข้อมูลเสริมในระบบ"}) มาตอบเสริมเพื่อความสมบูรณ์ได้ โดยใส่หมายเลข [2], [3] กำกับท้ายข้อความที่มาจากแหล่งเสริมนั้นๆ\n` +
      ...
      `4. **การใส่หมายเลขกำกับในเนื้อหา [1], [2] และรายการอ้างอิงทุกแหล่ง (In-Text Citations & All-Source APA):**\n` +
      `   - ให้ใส่หมายเลขในวงเล็บก้ามปู เช่น [1] หรือ [2] กำกับท้ายประโยคสำคัญในเนื้อหาคำตอบ เพื่อให้ผู้ใช้สามารถสืบค้นย้อนกลับไปยังเอกสารต้นทางได้ทันที\n` +
      `   - ในหัวข้อ '### 📚 เอกสารอ้างอิง (APA 7th Edition)' ต้องเขียนรายการเอกสารอ้างอิงของ**ทุกแหล่งข้อมูลที่นำมาใช้ตอบจริงให้ครบถ้วน** โดยเรียงลำดับตัวเลข 1, 2, 3 ให้ตรงกับ [1], [2], [3] ในเนื้อหา (ระบุแหล่งหลัก "${apaCite}" เป็นอันดับแรก [1] เสมอ และตามด้วยเอกสารอ้างอิงเสริม [2], [3] ทั้งหมดที่นำมาใช้ตอบจริงต่อท้าย)`;
  } else if (isDdi) {
    contextText =
      ...
      contextText;
    dynamicInstructions += "\n\n⚠️ **กฎสำคัญสูงสุดเรื่องอันตรกิริยาระหว่างสมุนไพรกับยา (Herb-Drug Interactions):** คำถามนี้เป็นคำถามเกี่ยวกับอันตรกิริยาระหว่างสมุนไพรกับยา หรือการกินสมุนไพรร่วมกับยาแผนปัจจุบัน **ต้องใช้แหล่งข้อมูลจาก 'คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ กรมการแพทย์ (พ.ศ. 2568)' ในการเป็นแหล่งข้อมูลในการตอบคำถามเท่านั้น** ห้ามเอ่ยถึง อ้างอิง หรือใช้ข้อมูลจาก มหาวิทยาลัยมหิดล, ศูนย์ข้อมูลสมุนไพร ม.มหิดล, มหาวิทยาลัยธรรมศาสตร์, ศ. ดร.ภญ.อรุณพร อิฐรัตน์ หรือ PubMed/ThaiJO โดยเด็ดขาด ให้ใส่หมายเลข [1] กำกับท้ายประโยคหรือตารางอันตรกิริยา และในหัวข้อ '### 📚 เอกสารอ้างอิง (APA 7th Edition)' ให้ระบุเฉพาะ '1. กรมการแพทย์. (2568). คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ. กระทรวงสาธารณสุข.' เท่านั้น";
  } else if (matched97Herbs.length > 0) {
    const h97Name = matched97Herbs[0].name;
    contextText =
      `\n[แหล่งข้อมูลหลักตามบัญชียาหลักแห่งชาติด้านสมุนไพร (NLEM Primary Authority)]:\n` +
      `• ยาที่สืบค้น: "${h97Name}" เป็นตำรับยา/สมุนไพรในบัญชียาหลักแห่งชาติด้านสมุนไพร (NLEM)\n` +
      `• แหล่งข้อมูลหลัก (Primary Core Authority [1]): "คณะกรรมการพัฒนาระบบยาแห่งชาติ. (2568). ประกาศคณะกรรมการพัฒนาระบบยาแห่งชาติ เรื่อง บัญชียาหลักแห่งชาติด้านสมุนไพร (ฉบับที่ 2) พ.ศ. 2568. ราชกิจจานุเบกษา."\n` +
      `• แหล่งข้อมูลเสริม (Supplementary Source [2]): "กรมการแพทย์แผนไทยและการแพทย์ทางเลือก. (2567). คู่มือการใช้ยาสมุนไพรในการดูแลสุขภาพเบื้องต้น 10 กลุ่มอาการ. กระทรวงสาธารณสุข."\n` +
      `• ⚠️ กฎเหล็ก Zero-Hallucination ด้านเอกสารอ้างอิง: ยานี้ไม่มีบันทึกใน CPG กรมการแพทย์ 2568 ดังนั้น **ห้ามอ้างอิง 'กรมการแพทย์. (2568). คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ' โดยเด็ดขาด** ให้ระบุเอกสารอ้างอิง [1] เป็นประกาศคณะกรรมการพัฒนาระบบยาแห่งชาติ และ [2] เป็นคู่มือ 10 กลุ่มอาการ กรมการแพทย์แผนไทยฯ เท่านั้น\n` +
      `• ⚠️ กฎเหล็ก Zero-Hallucination ด้านข้อบ่งใช้: ให้ตอบข้อบ่งใช้ตามที่ระบุใน [ข้อมูลเภสัชกรรมและแนวทางการใช้ยาสมุนไพร] ด้านบนอย่างถูกต้องตรงไปตรงมา ห้ามนำข้อบ่งใช้เรื่องระบบทางเดินอาหาร ท้องอืด ท้องเฟ้อ หรือบำรุงธาตุของยาอื่นมาปะปนเด็ดขาด\n\n` +
      contextText;
    dynamicInstructions += `\n\n⚠️ **กฎสำคัญเรื่องการอ้างอิงสำหรับ ${h97Name}:** ยานี้เป็นยาในบัญชียาหลักแห่งชาติด้านสมุนไพร ในหัวข้อ '### 📚 เอกสารอ้างอิง (APA 7th Edition)' ให้ระบุรายการที่ 1 คือประกาศคณะกรรมการพัฒนาระบบยาแห่งชาติ พ.ศ. 2568 [1] และห้ามอ้างอิง 'กรมการแพทย์. (2568). คู่มือการใช้ยาสมุนไพรในเวชปฏิบัติ' เด็ดขาด`;
  }
  ...
  if (currentSettings.show_apa_citations === false) {
    dynamicInstructions += "\n\n⚠️ **คำสั่งสำคัญเรื่องเอกสารอ้างอิง:** ขณะนี้ระบบปิดการแสดงผลหัวข้อเอกสารอ้างอิง APA 7th Edition ห้ามใส่หัวข้อ '### 📚 เอกสารอ้างอิง (APA 7th Edition)' หรือรายการอ้างอิง APA ใดๆ ท้ายคำตอบเด็ดขาด และห้ามใส่หมายเลขกำกับอ้างอิง [1], [2] ในเนื้อหาคำตอบ";
  }
